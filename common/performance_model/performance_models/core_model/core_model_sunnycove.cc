...
static unsigned int bypassLatencies[DynamicMicroOpSunnycove::UOP_BYPASS_SIZE];

// Intel Sunnycove Latencies
// http://www.agner.org/optimize

CoreModelSunnycove::CoreModelSunnycove()
{
   ...
   bypassLatencies[DynamicMicroOpSunnycove::UOP_BYPASS_NONE] = 0;
   bypassLatencies[DynamicMicroOpSunnycove::UOP_BYPASS_LOAD_FP] = 2;
   bypassLatencies[DynamicMicroOpSunnycove::UOP_BYPASS_FP_STORE] = 1;


   m_lll_cutoff = Sim()->getCfg()->getInt("perf_model/core/interval_timer/lll_cutoff");
}

unsigned int CoreModelSunnycove::getInstructionLatency(const MicroOp *uop) const
{
...
unsigned int CoreModelSunnycove::getAluLatency(const MicroOp *uop) const
{
...
unsigned int CoreModelSunnycove::getBypassLatency(const DynamicMicroOp *uop) const
{
   const DynamicMicroOpSunnycove *info = uop->getCoreSpecificInfo<DynamicMicroOpSunnycove>();
   DynamicMicroOpSunnycove::uop_bypass_t bypass_type = info->getBypassType();
   LOG_ASSERT_ERROR(bypass_type >=0 && bypass_type < DynamicMicroOpSunnycove::UOP_BYPASS_SIZE, "Invalid bypass type %d", bypass_type);
   return bypassLatencies[bypass_type];
}

unsigned int CoreModelSunnycove::getLongestLatency() const
{
   return 60;
}

IntervalContention* CoreModelSunnycove::createIntervalContentionModel(const Core *core) const
{
   return new IntervalContentionSunnycove(core, this);
}

RobContention* CoreModelSunnycove::createRobContentionModel(const Core *core) const
{
   return new RobContentionSunnycove(core, this);
}

DynamicMicroOp* CoreModelSunnycove::createDynamicMicroOp(Allocator *alloc, const MicroOp *uop, ComponentPeriod period) const
{
   DynamicMicroOpSunnycove *info = DynamicMicroOp::alloc<DynamicMicroOpSunnycove>(alloc, uop, this, period);
   info->uop_port = DynamicMicroOpSunnycove::getPort(uop);
   info->uop_bypass = DynamicMicroOpSunnycove::getBypassType(uop);
   info->uop_alu = DynamicMicroOpSunnycove::getAlu(uop);
   return info;
}
